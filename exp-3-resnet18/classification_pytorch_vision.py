...
def main(args):
    ...
    # device = torch.device(args.device) # from original code
    device = exp.device
    ...
    if (exp.ml == "training") and exp.name_calc != 'flops':
        ...
        # tracker = get_calculator(args.epochs, args.calculator) 

        # print("Start training")
        # start_time = time.time()

        # if args.calculator != "carbon_tracker" and args.calculator != None and args.calculator != "impact_tracker":
        #     tracker.start()
        # if args.calculator == "impact_tracker":
    ...
    if (exp.ml == "inference") and not exp.name_calc == 'flops':
        ...
        # PATH = os.path.join(_path, "models", "ResNet_imageNet.pth")
        # model.load_state_dict(torch.load(PATH))
        
        for kk in tqdm(range(args.nb_batch_inferences)):
    ...
    Ec_kWh = flops_method_pytorch(exp, data_loader, copy_model)

    # Saving the data:
    if exp.name_calc == 'code_carbon':
        save_cc(exp, args, duration)
    elif exp.name_calc == 'carbon_tracker':
        save_ct(exp, args, duration)
    elif exp.name_calc == 'eco2ai':
        save_eco2ai(exp, args, duration)
        ...
        save_ga(exp, args, duration, exp.automated, cpu_util, gpu_util, ram_util)
    elif exp.name_calc == 'tapo':
        stop_TAPO(exp, t0, tfinal)
        save_tapo(exp, args)
    elif exp.name_calc =='flops':
        save_FLOPS(exp, args, Ec_kWh)
    else: # no calculator
        save_nocalc(exp, args, duration)
    # ----------------------
...
def get_args_parser(add_help=True):
    ...
    # parser.add_argument("--calculator", default=None, type=str, help="calculator to meassure energy usage (Use carbon_tracker or code_carbon or eco2ai)")
    ...
    parser.add_argument("--nb_batch_inferences", type=int, help = "TODO", default = 20)
    parser.add_argument("--name_exp", type=str, help = "TODO", default = 'CUB_200_2011')
