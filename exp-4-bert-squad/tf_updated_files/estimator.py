...
# epochs = os.getenv('NUMBER_OF_EPOCHS', 2)

# output_file = os.getenv('OUTPUT_ESTIMATOR')
# tracker = CarbonTracker(epochs=epochs, update_interval = 0.01, log_dir = output_file, monitor_epochs = -1, epochs_before_pred = 0, decimal_precision = 10)
    ...
    with tf.Graph().as_default() as g:
      ...
      with strategy.scope(), scale_ctx:
        ...
        estimator_spec = model_fn_lib.EstimatorSpec(
            ...
            scaffold=scaffold)
        return self.c(estimator_spec, worker_hooks,
                                               hooks, global_step_tensor,
                                               saving_listeners)
  ...
  def _train_with_estimator_spec(self, estimator_spec, worker_hooks, hooks,
    ...
    with training.MonitoredTrainingSession(
      ...
      if carbon_tracker is not None:
        # -------- CT ------- #
        print('x ------------------------------------------------- x')
        steps_per_epoch = int(training_steps / training_epochs)
