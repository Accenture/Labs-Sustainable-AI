...
from fct_for_saving import save_ES
...
from fct_for_experiments import flops_method_tensorflow
# ---------------------
...
# flags.DEFINE_string("calculator", None,
#                     "calculator to meassure energy usage (Use carbon_tracker or code_carbon or eco2ai or impact_tracker)")
...
flags.DEFINE_string(
  ...
  "Calculator used to meassure energy usage."
  )
flags.DEFINE_string(
  "calculator_mode", "",
  "Specify the mode for some calculators ."
  )
flags.DEFINE_integer(
  "nb_batch_inferences", 1, 
  "Number of inferences we track with the calculator"
  )
flags.DEFINE_string(
  "name_exp", "SQUAD-extracted", 
  "Name of the ML experiment we are evaluating."
  )
flags.DEFINE_string(
  "computer", "linux_alienware", 
  "Computer on which we are running the experiment."
  )
flags.DEFINE_string(
  "path_logs_and_results", ".", 
  "Folder in which we store logs and calculators' outputs."
  )
flags.DEFINE_string(
...
def main(_):
  ...
  if FLAGS.use_tpu and FLAGS.tpu_name:
    tpu_cluster_resolver = slim.cluster_resolver.TPUClusterResolver(
  ...
  if (exp.ml == "training") and exp.name_calc != 'flops':
    ...
    tmp_model_foder_name = os.path.join(exp.path_logs_and_results, exp.name+"_model")
    for f in os.listdir(tmp_model_foder_name):
      os.remove(os.path.join(tmp_model_foder_name, f))
  ...
  if (exp.ml == "training") and exp.name_calc != 'flops':
    ...
    if FLAGS.calculator == "carbon_tracker":   
      estimator.train(
  ...
  elif (exp.ml == "inference") and not exp.name_calc == 'flops':
  ...
  train_examples = read_squad_examples(
        input_file=FLAGS.train_file, is_training=False)
  nb_examples = len(train_examples)
  graph_folder = os.path.join("models", "bert_saved_model")
  Ec_kWh = flops_method_tensorflow(exp, nb_examples, graph_folder)


  # Saving the data:
  if exp.name_calc == 'code_carbon':
      save_cc(exp, FLAGS, duration)
  elif exp.name_calc == 'carbon_tracker':
      save_ct(exp, FLAGS, duration)
  elif exp.name_calc == 'eco2ai':
      save_eco2ai(exp, FLAGS, duration)
    ...
    save_ga(exp, FLAGS, duration, exp.automated, cpu_util, gpu_util, ram_util)
  elif exp.name_calc == 'tapo':
      stop_TAPO(exp, t0, tfinal)
      save_tapo(exp, FLAGS)
  elif exp.name_calc =='flops':
      save_FLOPS(exp, FLAGS, Ec_kWh)
  else: # no calculator
      save_nocalc(exp, FLAGS, duration)
  # ----------------------
