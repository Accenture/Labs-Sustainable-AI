...
def get_calculator(epochs: int, calculator: str = None):
    if calculator is None:
        print("No calculator is about to be used during the training or inference.")
        specific_calculator = None
    elif calculator == "code_carbon":
        output_file = "output/calculator_output/output_cc_resnet.csv"
        specific_calculator = EmissionsTracker(output_file = output_file)
    elif calculator == "carbon_tracker":
        update_interval = 0.01  
        monitor_epochs = -1     
        epochs_before_pred = 0  
        decimal_precision = 10

        specific_calculator = CarbonTracker(epochs=epochs, 
                            update_interval = update_interval, 
                            log_dir = "output/calculator_output/", 
                            monitor_epochs = monitor_epochs, 
                            epochs_before_pred = epochs_before_pred, 
                            decimal_precision = decimal_precision)
    elif calculator == "eco2ai":
        output_file = "output/calculator_output/output_eco2ai.csv"
        specific_calculator = eco2ai.Tracker(file_name=output_file, alpha_2_code="FR")

    elif calculator == "impact_tracker":
        output_file = "output/calculator_output/"
        specific_calculator = ImpactTracker(output_file)

    return specific_calculator
...
def main(_):
  ...
  if FLAGS.do_train:
    ...
    tracker = get_calculator(FLAGS.num_train_epochs, FLAGS.calculator)

    if FLAGS.calculator != "carbon_tracker" and FLAGS.calculator != None and FLAGS.calculator != "impact_tracker":
        tracker.start()
    if FLAGS.calculator == "impact_tracker":
    ...
    if FLAGS.calculator == "carbon_tracker":
            tracker.epoch_end()
    if FLAGS.calculator != None and FLAGS.calculator != "impact_tracker"and FLAGS.calculator != "carbon_tracker":
        tracker.stop()
